...
- 同步机制（推送驱动，无轮询，无 ACK 等待）：
  ...
  3. 不做 ACK 等待/重传；若推送失败，从机只在“冷启动 / 重新回到前台且临近下次提醒”时同步（高频打开不重复同步），另有“每天最早一次提醒前 2 分钟”定时拉取兜底（`ENABLE_DAILY_FALLBACK` 常量可关，每天最多 1 次）
...
5. 超时：方案1 下 `set-config` 写入+推送后立即返回，用默认超时即可；此前调的 60 秒可改回默认。
